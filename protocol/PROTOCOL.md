...
## 10. One trunk, many clones

The plugin runs in several environments and updates itself in each (refreshes, learnings, environment facts, test runs). The trunk is one git repository on a host (`git.upstream` in `evergreen.config.json`; GitHub by default), and every install is a clone of it. Two roles, decided by what the host lets a clone do, never by the agent:
...
- Secrets and scope. `git add -A` runs inside the plugin root only; `.gitignore` keeps archives and caches out. The repository holds `profile/` (the owner's preferences and environment facts) and `evergreen.config.json` (paths, the owner's address), so it stays private; a copy for other people is `pack --share`, or a fork that strips `profile/`. Only the plugin publishes; other units do not (their facts travel inside `profile/` when they are profile facts, or not at all).
