...
- `evals/evals.json`: the cases, in skill-creator's `evals.json` shape (`skill`, `evals: [{id, prompt, files, expectations}]`) plus evergreen fields: `kind`, `decoy`, `evidence` (`{type: trace|file|marker|command|log, tool, input_match, path, ...}`), `baseline`, `runs`. skill-creator's runner and `claude plugin eval` ignore the extra fields, so one suite serves every harness. Template: `templates/evals.json.template`.
...
|---|---|---|---|
| Claude Code, `claude plugin eval` actually runs | `claude plugin eval <plugin> --case <glob> --runs 3 --json`; graders `tool_used` (tool `Skill`, `input_match` on the skill name), `file_exists`, `regex`, `tool_order`, `llm` (flags as documented on 2026-09-04) | the harness's per-run trace and grader results | Undocumented early access: withdrawn from the public docs by 2026-09-10 (only `claude plugin validate` remains), so prefer the next two rows unless the command runs; the suite directory is `evals/<case>/prompt.md` plus `graders/`, so keep `evals.json` canonical and generate the case folders from it when this harness is used |
