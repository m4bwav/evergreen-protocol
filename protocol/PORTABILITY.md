...
- `AGENTS.md` at a repo root is the cross-tool instruction surface (Linux Foundation standard; read by Codex, Cursor Agent mode, Copilot, OpenCode, Amp, Windsurf, and Gemini CLI with one config line). `CLAUDE.md` contains the line `@AGENTS.md` (an import) plus Claude-only notes. Claude Code 2.1.277 and later reads `AGENTS.md` by itself only when no `CLAUDE.md` or `CLAUDE.local.md` sits in the working directory or above it, and a `CLAUDE.md` that tells Claude in words to read `AGENTS.md` loads nothing; the import works in every version and in the sessions that cannot read `AGENTS.md` directly (Bedrock, telemetry off, the first session after an upgrade).
