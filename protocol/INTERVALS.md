...
Part of the [Evergreen Protocol](PROTOCOL.md). Decides how often a unit re-researches its topic. The schedule adapts: it shortens when a check finds real change, lengthens gently when nothing changed, and stays inside bounds set by the topic's volatility tier. Topics can go fast, then slow, then fast again; the tier migration rules handle that. Evidence: [../RESEARCH.md](../RESEARCH.md) R-20260901-2 (the shape), R-20260923-10 and R-20260923-12 (the benchmark that set the step sizes in protocol 1.11).
...
2. m ≥ 0.6: I = I ÷ 2. If that lands below the tier's min and the tier is `moderate` or slower, promote one tier now (see migration) and keep I = max(new min, I ÷ 2).
3. 0.3 ≤ m < 0.6: I = I ÷ 1.5 (two thirds of I)
...
Worked example (`moderate`, 14 to 90 days, rounded for display): a quiet field walks 30 → 37.5 → 47 → 59 → 73 → 90 and settles at the cap. A shake-up (m = 0.7) pulls 90 → 45; quiet checks then walk 56 → 70 → 88 → 90. Five consecutive real changes (m = 0.4): 90 → 60 → 40 → 27 → 18 → 14 (pinned at min, see migration). A `slow` unit at 120 days that finds a core claim wrong (m = 0.9) halves to 60 days, the tier's floor, and stays `slow`; a second material change on the next check pins it there again and promotes it to `moderate` at 14 days (migration). A `glacial` unit at 365 days with the same finding lands at 182 days, below its floor of 270, and becomes `slow` at 182 days in one check.

## Tier migration

Topics change character. After each check:

- Immediate promotion: a major change (m ≥ 0.6) whose halved interval falls below the tier's min moves the unit one tier faster right away. `fast` and `live` have nowhere faster to go; they pin at min and count.
