...
`EG checked <unit> --m <m> --note "<one line>; cost: searches n · fetches n · tool calls n"`, with the research pass's cost line in the note so the history shows what each refresh spent (inline research: count your own calls). It applies the interval rule, tier migration, event caps, jitter, and history, and clears any contradiction flag. Searches failed entirely? Run it with no `--m` so the schedule stays put and retries next use.

A `verify_at_use` unit checked at use time re-checks only its due claims: `EG claims <unit> --due` lists them by number, one targeted search each, then `EG claims <unit> --stamp due` and `EG checked <unit> --m <m> --use-time`. A claim that changed is edited in the main file like any finding; a new volatile claim goes in with `EG claims <unit> --add "<claim>"`.
