...
| `hooks/` | Claude Code hooks: `SessionStart` prints what needs attention into context (units due for refresh, verify-at-use units with claims due, units whose learnings passed `consolidate_every`; silent when nothing does); `SessionEnd` publishes any self-update to the trunk repository in a detached process; `PostToolUse` on the `Skill` tool writes the use log. |
