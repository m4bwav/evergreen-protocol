...
- This repo is the trunk (protocol 1.5, `PROTOCOL.md` §10). Log entries, state and claim edits may go straight to `master` from a maintainer's clone (`python scripts/evergreen.py publish`); scripts, hooks and protocol changes go through a pull request (`publish --branch`). Never force-push; never rewrite `master`.
